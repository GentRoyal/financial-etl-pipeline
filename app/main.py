...
async def load_application_dependencies():
    ...
    try:
        ...
        model_path = os.path.join(base_path, 'models', 'model_20250521_074410.pkl')
        result_path = os.path.join(base_path, 'data', 'results', 'results_20250521_074410.csv')
...
async def make_predictions_from_input(
    data: DataPoint,
...
async def historical_prediction(
    symbol: str = Query(..., description='Enter Stock Symbol (e.g. AAPL)'),
    api_key: str = Depends(get_api_key)
):
    try:
        ...
        df = tech_indicator.generate_indicators(symbol)
        df = df[REQUIRED_FEATURES]

        logger.info(f"Technical Indicators Generated successfully")
